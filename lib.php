 ...
 *
 * @since Moodle 2.0
 * @package    repository_uploadoptimized
...
class repository_uploadoptimized extends repository {
     ...
     * @param null $license optional the license to use for this file
    ...
    public function process_upload($saveasfilename, $maxbytes, $types = '*', $savepath = '/', $itemid = 0,
        ...
        if (!empty($_FILES[$elname]['error'])) {
            switch ($_FILES[$elname]['error']) {
                case UPLOAD_ERR_INI_SIZE:
                    throw new moodle_exception('upload_error_ini_size', 'repository_uploadoptimized');
                    break;
                case UPLOAD_ERR_FORM_SIZE:
                    throw new moodle_exception('upload_error_form_size', 'repository_uploadoptimized');
                    break;
                case UPLOAD_ERR_PARTIAL:
                    throw new moodle_exception('upload_error_partial', 'repository_uploadoptimized');
                    break;
                case UPLOAD_ERR_NO_FILE:
                    throw new moodle_exception('upload_error_no_file', 'repository_uploadoptimized');
                    break;
                case UPLOAD_ERR_NO_TMP_DIR:
                    throw new moodle_exception('upload_error_no_tmp_dir', 'repository_uploadoptimized');
                    break;
                case UPLOAD_ERR_CANT_WRITE:
                    throw new moodle_exception('upload_error_cant_write', 'repository_uploadoptimized');
                    break;
                case UPLOAD_ERR_EXTENSION:
                    throw new moodle_exception('upload_error_extension', 'repository_uploadoptimized');
                    break;
                default:
                    throw new moodle_exception('nofile');
            }
        ...
        // Scale down oversized images so the long side does not exceed the given size.
        $this->downscale_image_if_needed($_FILES[$elname]['tmp_name'], 512);
        ...
        if (!$this->check_valid_contents($_FILES[$elname]['tmp_name'])) {
            throw new moodle_exception('upload_error_invalid_file', 'repository_uploadoptimized', '', $record->filename);
        }
    ...
    /**
     * Downscale an uploaded image in-place so its long side does not exceed $maxlongside pixels.
     *
     * Non-image files and images that already fit within the limit are left untouched.
     * Supported formats: JPEG, PNG, GIF, WebP (requires the GD extension).
     *
     * @param string $filepath  Absolute path to the temp file to inspect / rewrite.
     * @param int    $maxlongside  Maximum allowed width or height in pixels.
     */
    protected function downscale_image_if_needed(string $filepath, int $maxlongside): void {
        // Check image and get dimensions.
        $info = @getimagesize($filepath);
        if ($info === false) {
            return; // Not a recognised image — leave unchanged.
        }

        [$width, $height, $imagetype] = $info;
        if (max($width, $height) <= $maxlongside) {
            return; // Already within the limit.
        }

        // Calculate target image scaling.
        $scale = $maxlongside / max($width, $height);
        $newwidth = max(1, (int) round($width * $scale));
        $newheight = max(1, (int) round($height * $scale));

        // Prepare GD resources for conversion.
        $src = $this->create_gd_image_from_file($filepath, $imagetype);
        if (!$src) {
            return; // Format not supported by GD — leave unchanged.
        }
        ...
        try {
            // Preserve alpha/transparency channel for PNG, GIF and WebP.
            ...
            $this->write_gd_image_to_file($dst, $filepath, $imagetype);
        } finally {
            // Cleanup.
            imagedestroy($dst);
            imagedestroy($src);
    ...
    /**
     * Create a GD image resource from a file, using the supplied IMAGETYPE_* constant.
     *
     * @param string $filepath
     * @param int    $imagetype  One of the PHP IMAGETYPE_* constants.
     * @return \GdImage|false
     */
    protected function create_gd_image_from_file(string $filepath, int $imagetype) {
    ...
    /**
     * Write a GD image resource back to disk at $filepath, re-encoding it to match $imagetype.
     *
     * @param \GdImage $image
     * @param string   $filepath
     * @param int      $imagetype  One of the PHP IMAGETYPE_* constants.
     ...
     */
    protected function write_gd_image_to_file($image, string $filepath, int $imagetype): bool {
        return match ($imagetype) {
            IMAGETYPE_JPEG => function_exists('imagejpeg') && @imagejpeg($image, $filepath, 85),
            IMAGETYPE_PNG => function_exists('imagepng') && @imagepng($image, $filepath, 9),
            IMAGETYPE_GIF => function_exists('imagegif') && @imagegif($image, $filepath),
            IMAGETYPE_WEBP => function_exists('imagewebp') && @imagewebp($image, $filepath, 85),
            default => false,
        };
    }
